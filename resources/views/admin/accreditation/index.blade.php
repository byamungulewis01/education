    ...
    <div class="container-xxl flex-grow-1 container-p-y">
        ...
        <div class="card">
            <div class="card-header border-bottom">
                <h5 class="card-title mb-0">Accreditation
                    <a class="btn btn-dark text-white pull-left float-end" href="{{ route('admin.accreditation.create') }}">
                        <i class="ti ti-plus me-0 me-sm-1 ti-xs"></i>
                        <span class="d-none d-sm-inline-block">New</span></a>
                </h5>
                ...
                <div class="row mb-5">
                    ...
                    <div class="col-md-6 col-lg-4 mb-3">
                        <div class="card h-100">
                            <img class="card-img-top" src="{{ asset('images/accreditation/' . $item->imageName) }}" alt="Card image cap">
                            <div class="card-body">
                                <h5 class="card-title">{{ $item->title }}</h5>
                                <p class="card-text">{!! Illuminate\Support\Str::limit(strip_tags($item->description), 130) !!}</p>
                                <a href="{{ route('admin.accreditation.edit',$item->id) }}" class="btn btn-outline-primary waves-effect">Edit</a>

                                <form id="deleteForm_{{ $item->id }}"
                                    action="{{ route('admin.accreditation.destroy', $item->id) }}" method="post"
                                    class="d-inline">
                                    @csrf
                                    @method('DELETE')
